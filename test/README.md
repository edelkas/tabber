...
| `test_core.c` | Strings, buffers, path handling, canonical paths, tab listing columns, timestamps, the message log, JSON, KeyValues, MD5 |
| `test_archive.c` | ZIP reading and writing, DEFLATE both ways, gzip, CRC verification, corruption, unsafe entry names |
| `test_state.c` | `config.json` round trips and lifecycle, the front-end's own settings, server resolution, URI budget, the server health check, when a tab was last played |
