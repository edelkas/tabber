...
A third thing is vendored beside them, and nothing of it is compiled:
[ForkAwesome](https://github.com/ForkAwesome/Fork-Awesome), an open fork of the
FontAwesome icon set, as two headers under `vendor/forkawesome/`. One holds the
TTF itself, compressed into an array; the other gives every codepoint a name —
`ICON_FK_INFO_CIRCLE` and its eight hundred siblings — and is generated by
[IconFontCppHeaders](https://github.com/juliettef/IconFontCppHeaders).
`build_font` merges the icons into ProggyClean at the same reference size, so
an icon can be written wherever a character can and a column of them lines up.
The build scripts only have to put the directory on the GUI's include path.
Keep that baked header in plain ASCII: `binary_to_compressed_c` redirected
through PowerShell writes UTF-16 instead, which MSVC reads by its byte order
mark and GCC and Clang cannot read at all.
...
Bump `TABBER_VERSION` and `TABBER_DATE` in `src/version.h`, build each platform, run that, then
