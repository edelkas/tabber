...
ifeq ($(OS),Windows_NT)
  ...
  GUILINK := -mwindows
  # The front-end does its work on a second thread. A GCC built for the win32
  # threads model has no std::thread to do it with; the posix one, which is
  # what MSYS2 and most MinGW-w64 packages ship, does.
  GUITHREADS :=
else ifeq ($(UNAME_S),Darwin)
  ...
  # The front-end's own worker thread, which wants the flag at both ends.
  GUITHREADS := -pthread
endif
...
$(GUITARGET): $(GUIOBJS) $(LIBOBJ)
	$(CXX) -o $@ $^ $(GUITHREADS) $(GUILIBS) $(GUILINK) $(LDLIBS)
	@mkdir -p $(DISTDIR)
	cp -f $@ $(DISTGUI)

$(GUIOBJ)/main.o: $(GUIDIR)/main.cpp | $(GUIOBJ)
	$(CXX) $(VENDORCXXFLAGS) $(GUITHREADS) -Wall -Wextra -c -o $@ $<
